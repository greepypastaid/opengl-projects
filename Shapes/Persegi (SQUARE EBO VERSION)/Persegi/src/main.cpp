...
int main() {
    ...
    // Tentukan koordinat 4 titik persegi aja sir!
    float vertices[] = {
        0.6f,  0.6f, 0.0f,   // 0: Atas kanan
        0.6f, -0.6f, 0.0f,   // 1: Bawah kanan
        -0.6f, -0.6f, 0.0f,   // 2: Bawah kiri
        -0.6f,  0.6f, 0.0f    // 3: Atas kiri
    };
    ...
    unsigned int VBO, VAO, EBO; // Tambahin EBO
    glGenVertexArrays(1, &VAO);
    glGenBuffers(1, &VBO);
    glGenBuffers(1, &EBO); // Tambahin EBO
    
    // Cara Inisialisasi VAO DLU -> VBO -> EBO YAH!
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        glDrawElements(GL_TRIANGLES, 6, GL_UNSIGNED_INT, 0); // Menggambar 6 indeks (INI WAJIB DIGANTI DARI glDrawArrays)
